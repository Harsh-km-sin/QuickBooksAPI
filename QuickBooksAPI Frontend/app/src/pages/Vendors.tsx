...
export function Vendors() {
  ...
  const handleOpenEditDialog = (vendor: Vendor) => {
    dispatch(openEditDialog(vendor));
  };

  const handleOpenDeleteDialog = (vendor: Vendor) => {
    dispatch(openDeleteDialog(vendor));
  };
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            <div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search vendors..." value={searchTerm} onChange={(e) => handleSearchChange(e.target.value)} className="pl-9" /></div>
        ...
        <CardContent>
          {vendors.length === 0 ? (
            <div className="text-center py-12"><Truck className="h-12 w-12 text-muted-foreground mx-auto mb-4" /><h3 className="text-lg font-semibold mb-2">No vendors found</h3><p className="text-muted-foreground mb-4">{debouncedSearch ? 'Try adjusting your search' : 'Get started by adding your first vendor'}</p>{!debouncedSearch && <Button onClick={() => dispatch(openCreateDialog())}><Plus className="h-4 w-4 mr-2" />Add Vendor</Button>}</div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {vendors.map((vendor) => (
                    <TableRow key={vendor.qboId}>
                      <TableCell><div className="font-medium">{vendor.displayName || `${vendor.givenName} ${vendor.familyName}`}</div></TableCell>
                      <TableCell>{vendor.companyName || '-'}</TableCell>
                      <TableCell>
                        <div className="space-y-1">
                          {vendor.primaryEmailAddr && <div className="flex items-center text-sm"><Mail className="h-3 w-3 mr-1 text-muted-foreground" />{vendor.primaryEmailAddr}</div>}
                          {vendor.primaryPhone && <div className="flex items-center text-sm"><Phone className="h-3 w-3 mr-1 text-muted-foreground" />{vendor.primaryPhone}</div>}
