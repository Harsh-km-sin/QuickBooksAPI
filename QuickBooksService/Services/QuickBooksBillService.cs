...
namespace QuickBooksService.Services
{
    public class QuickBooksBillService : IQuickBooksBillService
    {
        ...
        public async Task<string> GetBillsAsync(string accessToken, string realmId, int startPosition = 1, int maxResults = 100, DateTime? lastUpdatedAfter = null)
        {
            ...
            {
                _logger.LogWarning("QBO Bill request failed. StatusCode={StatusCode}, RealmId={RealmId}", response.StatusCode, realmId);
        ...
        public async Task<string> CreateBillAsync(string accessToken, string realmId, string billPayload)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("QBO Bill create failed. StatusCode={StatusCode}, RealmId={RealmId}", response.StatusCode, realmId);
                throw new HttpRequestException(
                    $"QBO request failed. Status={(int)response.StatusCode} {response.ReasonPhrase}.");
            }
            return content;
        ...
        public async Task<string> UpdateBillAsync(string accessToken, string realmId, string billPayload)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("QBO Bill update failed. StatusCode={StatusCode}, RealmId={RealmId}", response.StatusCode, realmId);
                throw new HttpRequestException(
                    $"QBO request failed. Status={(int)response.StatusCode} {response.ReasonPhrase}.");
            }
            return content;
        ...
        public async Task<string> DeleteBillAsync(string accessToken, string realmId, string billPayload)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("QBO Bill delete failed. StatusCode={StatusCode}, RealmId={RealmId}", response.StatusCode, realmId);
                throw new HttpRequestException(
                    $"QBO request failed. Status={(int)response.StatusCode} {response.ReasonPhrase}.");
            }
            return content;
